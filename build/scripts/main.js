...
const d = document;
let tool = d.querySelectorAll(".hex-item");
let itemId;
let lastToolId = tool[tool.length - 1].id;
...
function showTool(toolId) {
    ...
    let clickedObject = toolItems.find(findId);
    let {title, description} = clickedObject;
    let html = `
...
for (let i = 0, len = tool.length; i < len; i++) {
